...
#' @param celltype_column Character describing name of column to cluster clonotypes across
#' @param sample_column Character describing name of column that specifies samples to distinguish ontogically unrelated but potentially identical clonotypes
#' @param clone_min Integer specifying minimum number fo clones for a clonotype to be included in analysis
...
#' @export
cluster_clonotypes <- function(seurat_object, clonotype_column, cell_type_column, sample_column, clone_min=5, plot=TRUE) {
  ...
  clones_to_include <- clone_count[clone_count$clone_count >= clone_min, clonotype_column]
